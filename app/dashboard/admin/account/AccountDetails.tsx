...
export const AccountDetails = () => {
  ...
  // ✅ Выносим используемые поля до хуков — фикс react-hooks/preserve-manual-memoization
  const accountUserId = selectedAccount?.userId;
  const accountProviderId = selectedAccount?.providerId;

  const linkedUser = useMemo(() =>
    user?.find(u => u._id === accountUserId),
    [user, accountUserId]
  );

  const linkedProvider = useMemo(() => {
    if (!accountProviderId || !providers?.length) return null;
    
    const idStr = typeof accountProviderId === 'object' && accountProviderId !== null
      ? String((accountProviderId as { _id: string })._id || accountProviderId)
      : String(accountProviderId);
      
    return providers.find(p => String(p._id) === idStr) || null;
  }, [accountProviderId, providers]);

  const accountAvatar = selectedAccount?.avatar;
  const linkedUserId = linkedUser?._id;

  const avatarSrc = useMemo(() => {
    if (!accountAvatar) return null;
    const baseVersion = avatarVersions?.[linkedUserId || ''] ?? 0;
    if (baseVersion === 0) return accountAvatar;
    try {
      const url = new URL(accountAvatar);
      url.searchParams.set('cache', baseVersion.toString());
      return url.toString();
    } catch {
      return accountAvatar;
    }
  }, [accountAvatar, linkedUserId, avatarVersions]);
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="relative w-24 h-24 rounded-full overflow-hidden">
            {avatarSrc ? (
              <Image fill src={avatarSrc} alt="Avatar" sizes="96px" className="object-cover rounded-full" unoptimized priority={false} />
            ) : (
              <UserAvatar
                size="xl"
                name={linkedUser ? (linkedUser.username || `${linkedUser.firstName} ${linkedUser.lastName}`.trim()) : 'Unknown'}
                email={linkedUser?.email || ''}
                avatarVersion={avatarVersions?.[linkedUserId || ''] ?? 0}
              />
            )}
          </div>
        ...
        <div className="space-y-3 pt-2">
          {[
            ...
            <div key={label} className="flex items-center justify-between gap-4 py-2.5 border-b border-foreground/5 last:border-0">
              ...
              <span className="text-sm text-right truncate font-medium" title={String(value)}>{String(value)}</span>
            </div>
          ))}
        </div>
          ...
          <button
            onClick={async () => {
              ...
              toast.promise(
                ...
                { loading: 'Deleting account and user...', success: 'Deleted successfully', error: 'Failed to delete' },
