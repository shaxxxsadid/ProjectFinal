// app/components/ui/modalChangePass.tsx
'use client';
...
export interface ModalChangePassProps {
    isOpen: boolean;
    onClose: () => void;
    userEmail: string;
...
const FloatInput = ({ label, value, onChange, show, onToggle, autoComplete, required }: FloatInputProps) => {
    ...
    return (
        <div className="relative pt-5 pb-1 transition-all duration-200">
            {/* Label — шрифт не меняется, только transform */}
            <label
                className={`absolute left-0 text-sm text-muted-foreground pointer-events-none origin-left top-5 transition-all duration-200 
            ease-in-out`}
                style={{
            ...
            <input
                ...
                className="w-full pb-2 pr-8 bg-transparent text-sm text-foreground outline-none border-none ring-0"
            />
