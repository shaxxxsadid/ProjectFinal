...
const gradientColors = {
  dark: ['#60a5fa', '#818cf8', '#a78bfa', '#c084fc', '#e879f9'],
  light: ['#3b82f6', '#6366f1', '#8b5cf6', '#a855f7', '#d946ef'],
};

export const BackgroundBeams = React.memo(({ className, theme = 'dark', intensity = 'medium' }: BackgroundBeamsProps) => {
  const [activeTheme, setActiveTheme] = useState<'light' | 'dark'>(theme);

  useEffect(() => {
    const saved = localStorage.getItem('theme');
    if (saved) setActiveTheme(saved as 'light' | 'dark');//eslint-disable-line
    else if (window.matchMedia('(prefers-color-scheme: dark)').matches) setActiveTheme('dark');
  }, [theme]);

  const colors = gradientColors[activeTheme];
  const opacity = { low: 0.3, medium: 0.5, high: 0.7 }[intensity];
  const staticOpacity = activeTheme === 'dark' ? 0.02 : 0.015;
        ...
        <defs>
          {pathData.map((_, i) => (
            <linearGradient
              ...
              y2="100%"
            >
              <stop offset="0%" stopColor={colors[i % colors.length]} stopOpacity="0" />
              <stop offset="20%" stopColor={colors[i % colors.length]} stopOpacity="1" />
              <stop offset="50%" stopColor={colors[(i + 2) % colors.length]} stopOpacity="1" />
              <stop offset="80%" stopColor={colors[(i + 3) % colors.length]} stopOpacity="1" />
              <stop offset="100%" stopColor={colors[(i + 4) % colors.length]} stopOpacity="0" />
            </linearGradient>
          ))}
        </defs>
      </svg>
